...
shuffle_index = np.random.permutation(60000)#随机全排列60000个数字
...
![图3-1](../images/chapter_3/1-1.png)

![图3-1](../images/chapter_3/1-2.png)

![图3-1](../images/chapter_3/1-3.png)
...
https://www.zhihu.com/question/21351965/answer/31050145
...
def stocGradAscent0(dataMatrix, classLabels):
    ...
    for i in range(m):
        ...
        h = sigmoid(sum(dataMatrix[i]*weights))#每次为一个随机数而非矩阵，计算量小
...
for train_index, test_index in skfolds.split(X_train, y_train_5):#拆分为二折训练，一折测试
    clone_clf = clone(sgd_clf)#sgd_clf 是SGD分类器
...
>>> cross_val_score(sgd_clf, X_train, y_train_5, cv=3, scoring="accuracy")#cv：交叉验证生成器或可迭代的次数
...
y_multilabel = np.c_[y_train_large, y_train_odd]#设置2个类别
