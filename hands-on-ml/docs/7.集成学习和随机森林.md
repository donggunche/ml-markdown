...
假设你已经训练了一些分类器，每一个都有 80% 的准确率。你可能有了一个logistic回归、或一个 SVM、或一个随机森林，或者一个 KNN，或许还有更多（详见图 7-1）
...
同样的，假设你创建了一个包含 1000 个同样的分类器的集成模型，其中每个分类器的正确率只有 51%（仅比瞎猜好一点点）。如果你用投票去预测类别，你可能得到 75% 的准确率！它们需要都独立运行、不会发生有相关性的错误的情况下才会这样，然而每一个分类器都在同一个数据集上训练，他们可能会犯同一种错误，所以也会导致集成的准确率下降。

如果使每一个分类器都独立自主的分类，分类器的使用完全不同的算法，那么集成模型会工作的很好，这会使它们会做出不同种类的错误，这会提高集成的正确率。
...
```
svm_clf = SVC(probability=True)#添加predict_proba()方法，进行软投票
voting_clf = VotingClassifier(estimators=[('lr', log_clf), ('rf', rnd_clf), 
...
换句话说，Bagging 和 Pasting 都允许对训练集进行多次采样，但 Bagging 允许对训练集进行多次放回采样。采样和训练过程如图7-4所示。
...
>>>from sklearn.tree import DecisionTreeClassifier#基分类器
>>>bag_clf = BaggingClassifier(DecisionTreeClassifier(), n_estimators=500,        
>>>  max_samples=100, bootstrap=True, n_jobs=-1) # 500 个决策树分类器
...
对于 Bagging 来说，一些实例可能被一些分类器重复采样，但其他的有可能不会被采样。`BaggingClassifier`默认是有放回的采样`m`个实例 （`bootstrap=True`），其中`m`是训练集的大小，这意味着平均下来只有63%的训练实例被每个分类器采样，剩下的37%个没有被采样的训练实例就叫做 *Out-of-Bag* 实例。注意对于每一个的分类器它们的 37% 不是相同的。
...
```python
>>> from sklearn.metrics import accuracy_score #准确率测算
>>> y_pred = bag_clf.predict(X_test) 
...
正如我们所讨论的，随机森林是决策树的一种集成，通常是通过 bagging 方法（有时是 pasting 方法）进行训练，通常用`max_samples`设置为训练集的大小。你可以使用更方便的RandomForestClassifier`（回归是`RandomForestRegressor`）。接下来的代码训练了带有 500 个树（每个被限制为 16 叶子结点）的决策森林，使用所有空闲的 CPU 核：
...
随机森林算法在树生长时引入了额外的随机，它在一个随机的特征集中找最好的特征。它导致了树的差异性，并且再一次用高偏差换低方差，总的来说是一个更好的模型。以下是`BaggingClassifier`大致相当于之前的`randomforestclassifier`：
...
你可以使用 sklearn 的`ExtraTreesClassifier`来创建一个 *Extra-Tree* 分类器。他的 API 跟`RandomForestClassifier`是相似的， *ExtraTreesRegressor* 跟`RandomForestRegressor`也是相同的 API。
...
让我们详细看一下 Adaboost 算法。每一个分类器的权重`wi`初始都被设为`1/m`，第一个分类器被训练后，然后他的权重误差率`r1`在训练集上算出，详见公式 7-1。
...
接下来的代码训练了使用 sklearn 的`AdaBoostClassifier`基于 200 个决策树桩 Adaboost 分类器（正如你说期待的，对于回归也有`AdaBoostRegressor`）。`max_depth=1`表示只有一个决策树桩的决策树。换句话说，是一个单一的决策节点加上两个叶子结点。这就是`AdaBoostClassifier`的默认基分类器：
...
让我们通过一个使用决策树当做基分类器的简单的回归例子。这被叫做梯度提升回归树（GBRT，*Gradient Tree Boosting* 或者 *Gradient Boosted Regression Trees*）。首先我们用`DecisionTreeRegressor`去拟合训练集（例如一个有噪二次训练集）：
